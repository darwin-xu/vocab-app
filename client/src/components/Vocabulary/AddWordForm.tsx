        ...
        <div className="bg-vocab-surface backdrop-blur-xl p-6 rounded-xl shadow-lg border border-white/20 mb-xl">
            <div className="flex gap-4 items-end">
                ...
                <Button
                    ...
                    disabled={selectedCount > 0 ? false : !query.trim() || disabled}
                    className={selectedCount > 0 ? "bg-gradient-danger hover:bg-red-600" : ""}
                >
                    {selectedCount > 0 ? `Remove (${selectedCount})` : 'Add Word'}
                </Button>
            </div>
        </div>
    );
