...
export function SessionDebugPanel({ onClose }: SessionDebugProps) {
    ...
    const getEventTypeColor = (type: LogoutEvent['type']) => {
        switch (type) {
            ...
            case 'auto': return 'text-yellow-600 bg-yellow-50';
            case 'server_error': return 'text-red-600 bg-red-50';
            case 'network_error': return 'text-orange-600 bg-orange-50';
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
                <div className="p-6 border-b border-gray-200">
                    <div className="flex justify-between items-center">
                        <h2 className="text-2xl font-bold text-gray-900">Session Debug Panel</h2>
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <span className={sessionInfo?.hasToken ? 'text-green-600' : 'text-red-600'}>
                                        {sessionInfo?.hasToken ? 'Authenticated' : 'Not Authenticated'}
                                    </span>
                                </div>
                                <div>
                                    <span className="font-medium">Username:</span> {sessionInfo?.username || 'N/A'}
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span className={sessionInfo?.healthCheckActive ? 'text-green-600' : 'text-red-600'}>
                                        {sessionInfo?.healthCheckActive ? 'Active' : 'Inactive'}
                                    </span>
                                </div>
                                <div>
                                    <span className="font-medium">Consecutive Failures:</span>{' '}
                                    <span className={(sessionInfo?.consecutiveFailures ?? 0) > 2 ? 'text-red-600' : 'text-gray-600'}>
                    ...
                    {sessionHealth && (
                        <div className="mb-8">
                            <h3 className="text-lg font-semibold mb-4">Session Health Overview</h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="bg-blue-50 p-4 rounded-lg">
                                    <div className="text-2xl font-bold text-blue-600">
                                        {sessionHealth.totalLogouts}
                                    </div>
                                    <div className="text-sm text-blue-600">Total Logouts</div>
                                </div>
                                <div className="bg-red-50 p-4 rounded-lg">
                                    <div className="text-2xl font-bold text-red-600">
                                        {sessionHealth.unexpectedLogouts}
                                    </div>
                                    <div className="text-sm text-red-600">Unexpected</div>
                                </div>
                                <div className="bg-green-50 p-4 rounded-lg">
                                    <div className="text-2xl font-bold text-green-600">
                                        {formatDuration(sessionHealth.averageSessionDuration)}
                                    </div>
                            ...
                            {sessionHealth.recentPatterns.length > 0 && (
                                <div className="mt-4">
                                    <h4 className="font-medium mb-2">Recent Patterns Detected:</h4>
                                    <ul className="list-disc list-inside space-y-1">
                                        {sessionHealth.recentPatterns.map((pattern: string, index: number) => (
                            ...
                            {sessionHealth.commonReasons.length > 0 && (
                                <div className="mt-4">
                                    <h4 className="font-medium mb-2">Common Logout Reasons:</h4>
                                    <div className="space-y-2">
                                        {sessionHealth.commonReasons.slice(0, 5).map((item: { reason: string; count: number }, index: number) => (
                    ...
                    <div className="mb-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-semibold">Logout History</h3>
                                    ...
                                    <div key={index} className="border rounded-lg p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="text-sm text-gray-500">
                                                Session: {formatDuration(event.sessionDuration)}
                                            </span>
                                        </div>
                                        ...
                                        {event.errorDetails && (
                                            <div className="text-sm text-red-600 mb-2">
                                                <strong>Error:</strong> {event.errorDetails}
                                            </div>
                                        )}
                                        {event.apiEndpoint && (
                                            <div className="text-sm text-gray-600 mb-2">
                                                <strong>Endpoint:</strong> {event.apiEndpoint}
                                                {event.httpStatus && (
