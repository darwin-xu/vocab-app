...
class SessionAnalytics {
    ...
    private storeEvent(event: LogoutEvent) {
        try {
            ...
            localStorage.setItem(SessionAnalytics.STORAGE_KEY, JSON.stringify(events));
        } catch (error) {
            console.warn('Failed to store logout event:', error);
        }
    }

    recordLogout(type: LogoutEvent['type'], reason: string, details?: {
        errorDetails?: string;
        apiEndpoint?: string;
        httpStatus?: number;
    }) {
    ...
    getSessionHealth(): {
        ...
        const avgDuration = events.length > 0 
            ? events.reduce((sum, e) => sum + e.sessionDuration, 0) / events.length
            : 0;
        ...
        if (recentEvents.filter(e => e.reason.includes('network')).length > 2) {
