...
- `npm run dev` — spins up the worker (`wrangler dev`) and client dev server concurrently; requires `OPENAI_TOKEN` in your shell.
...
## Environment & Secrets
Export `OPENAI_TOKEN` before running `npm run dev` or backend tests that touch the OpenAI gateway, e.g. `export OPENAI_TOKEN=...`. Use `.env.local` (ignored by git) to persist secrets for the client dev server, and avoid committing changes to `wrangler.toml` variables without confirming staging values.
